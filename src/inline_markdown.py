...
def split_nodes_link(old_nodes):
    ...
    for node in old_nodes:
        ...
        original_text = node.text
        image_nodes = extract_markdown_links(original_text)
        if len(image_nodes) == 0:
            nodes_list.append(node)
            continue
        ...
        for image in image_nodes:
            ...
            if len(sections) != 2:
                raise ValueError("Invalid markdown: missing closing delimiter in link markdown")
            if sections[0] != "":
                nodes_list.append(TextNode(sections[0], TextType.TEXT))
...
def text_to_textnodes(text):
    ...
    nodes_list = split_nodes_delimiter(initial_node,'**',TextType.BOLD)
